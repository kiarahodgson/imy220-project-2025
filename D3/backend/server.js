...
// Remove placeholder auth router; use DB-backed auth endpoints defined below
// app.use('/api/auth', require('./routes/auth'));

const STATIC_DIR = path.join(__dirname, 'public');
app.use(express.static(STATIC_DIR));

// Uploads directory (for project files)
...
// --- USERS ---
// Upload/replace user avatar (max 5MB). Accepts field name 'avatar'.
app.post('/api/users/:id/avatar', requireDb, multer({ storage, limits: { fileSize: 5 * 1024 * 1024 } }).single('avatar'), async (req, res) => {
  try {
    ...
    // Best-effort cleanup of previous avatar if stored locally
    try {
...
app.post('/api/checkins', requireDb, upload.array('files', 50), async (req, res) => {
  try {
    ...
    const now = new Date();
    // build files list from JSON and uploaded multipart
    ...
    // Optional list of filenames to remove on check-in
    let removeFiles = [];
    ...
    const doc = {
      ...
      message,
      type,
      // Only allow explicit version changes on check-in; otherwise reflect current project version
    ...
    // Only bump project version on check-in
    ...
    if (type === 'check-in') {
      ...
      // Merge newly uploaded files (replace by name)
...
// Upload/replace project image (max 5MB). Accepts field name 'image'.
app.post('/api/projects/:id/image', requireDb, multer({ storage, limits: { fileSize: 5 * 1024 * 1024 } }).single('image'), async (req, res) => {
  try {
    ...
    // Best-effort cleanup of previous image if stored locally
    try {
...
// List files for a project (public)
...
// Download a single file by name (increments download counter)
app.get('/api/projects/:id/files/:name', requireDb, async (req, res) => {
  try {
    ...
    // Increment download counts (best-effort): per-file and per-project total
    await Projects.updateOne(
...
// --- PROJECT DISCUSSIONS ---
// Members-only: view and post comments on a project's discussion board
...
// Unfriend: remove a mutual friendship between two users
app.delete('/api/friends', requireDb, async (req, res) => {
  try {
    ...
    // Clean up any pending friend requests between the two
...
app.post('/api/saves', requireDb, async (req, res) => {
  try {
    ...
    // Log activity: project saved (non-fatal on failure)
    try {
      await Checkins.insertOne({
        projectId: toId(projectId),
        userId: toId(userId),
...
// --- AUTH (simple for D2: username/email/_id) ---
...
app.post('/api/auth/login', requireDb, async (req, res) => {
  try {
    ...
    // No sessions/tokens needed for D2 — return the user
...
async function connectDb() {
  try {
    ...
    setTimeout(connectDb, 8000); // retry during dev
  }
}
